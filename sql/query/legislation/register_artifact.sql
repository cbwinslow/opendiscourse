WITH latest AS (
  ...
  UPDATE ingest.artifact a SET remote_url=%(remote_url)s::text, status=%(status)s::text, metadata=a.metadata || COALESCE(%(metadata)s::jsonb,'{}'::jsonb)
  FROM latest l WHERE a.artifact_id=l.artifact_id AND %(action)s::text='retry' RETURNING a.*
), append_version AS (
  INSERT INTO ingest.artifact(dataset_id,remote_url,local_path,artifact_key,artifact_version,period_start,period_end,content_type,bytes_downloaded,checksum_sha256,status,metadata)
  SELECT %(dataset_id)s::text,%(remote_url)s::text,%(local_path)s::text,%(artifact_key)s::text,
    COALESCE((SELECT artifact_version+1 FROM latest),1),%(period_start)s::date,%(period_end)s::date,%(content_type)s::text,%(bytes_downloaded)s::bigint,%(checksum_sha256)s::text,%(status)s::text,COALESCE(%(metadata)s::jsonb,'{}'::jsonb)
