...
-- reason that applies: the dataset is ranked nowhere, the kind is ranked nowhere (or a displayed
-- kind has no display row), or this dataset is not ranked for this kind. A ranked kind that is
-- not displayed, such as roster, is placed and does not fail resolve. Empty means every
-- assertion has a rank.
SELECT CASE
         ...
         WHEN NOT EXISTS (SELECT 1 FROM catalog.attribute_precedence p
           ...
           OR (a.name_kind IN ('official', 'common') AND NOT EXISTS (
