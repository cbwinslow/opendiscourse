...
grant rewritten. The `BEFORE UPDATE` trigger applies to every role including the owner and has two rules: any change to
`name_source_id` is always refused outside the resolver, and a change to the name columns is refused only for a row that
already has a `name_source_id`, because three geography writers still overwrite `name`; the geography loader story makes
the second rule unconditional. It stops accidental writes, not a determined one: any session can `SET LOCAL opendiscourse.resolver = 'on'`.
