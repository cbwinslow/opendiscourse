...
| People and identifiers | `core.person`, `core.person_identifier` (congress-legislators) | 12,771 people; BioGuide 12,770, ICPSR 12,298, LIS 328 (only senators have one; all 254 senators since 2003 are covered) |
| Member terms, state and district posts | `core.membership`, `core.post`, `core.division` (Story 3.3) | 45,535 terms (1789-present), 740 posts, 690 divisions; `coverage` memberships 100% for Congresses 108-119 |
...
Empty and waiting: `core.document_chunk`, `core.embedding`.
...
| Senate.gov roll-call XML | every Senate roll call and vote | LIS id (every senator since 2003 has one in `core.person_identifier`; checked 254 of 254) |
...
| congress-legislators terms | terms, party, state, district: **loaded (Story 3.3)**; committee membership (current only): not yet | BioGuide |
...
1. Done (Story 3.3): member terms, posts and divisions from congress-legislators (`coverage`
   memberships 100%). Still to do from the same repo: current committee membership. There is no LIS
   gap to fix: LIS ids exist only for senators, and every senator since 2003 has one.
