...
def _scope(plan: dict[str, Any]) -> set[str]:
    ...
    if unknown or not layers:
        raise ValueError(f"Choose one or more supported TIGER layers: {sorted(LAYER_INFO)}")
    return layers
...
def stage_tiger(plan: dict[str, Any], update: Callable[[str], None] | None = None) -> int:
    ...
    try:
        ...
        raise RuntimeError("TIGER loading requires the spatial extra: `uv sync --extra spatial`") from exc
    total = 0
    with connect() as conn:
        for item in plan["artifacts"]:
            ...
            geography_type, geoid_key, name_key, state_key, county_key = LAYER_INFO[layer]
            ...
            for start in range(0, int(info["features"]), 1_000):
                ...
                frame = pyogrio.read_dataframe(Path(artifact["local_path"]), skip_features=start, max_features=1_000)
                rows = []
                for offset, (_, feature) in enumerate(frame.iterrows()):
                    ordinal = start + offset + 1
                    raw = {str(key): (None if value is None else str(value)) for key, value in feature.drop(labels="geometry").items()}
                    geoid = raw.get(geoid_key)
                    if not geoid:
                        raise ValueError(f"TIGER {layer} row {ordinal} has no {geoid_key}")
                    geometry = feature.geometry
                    if geometry is None or geometry.is_empty:
                        continue
                    rows.append((artifact["artifact_id"], layer, ordinal, geoid, raw.get(name_key), raw.get(state_key) if state_key else None, raw.get(county_key) if county_key else None, Jsonb(raw), bytes(geometry.wkb)))
                if rows:
                    with conn.cursor() as cur:
                        cur.executemany("INSERT INTO stage.tiger_feature (artifact_id, layer, source_ordinal, geoid, name, state_fips, county_fips, raw, geom) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,ST_Transform(ST_SetSRID(ST_GeomFromWKB(%s),4269),4326)) ON CONFLICT DO NOTHING", rows)
                    total += len(rows)
            conn.commit()
    return total


def load_tiger(plan: dict[str, Any], update: Callable[[str], None] | None = None) -> int:
    ...
    layers = list(_scope(plan)); vintage = int(plan["selection"]["boundary_vintage"])
    if update:
        update("Creating TIGER geographies and boundaries")
    with connect() as conn, conn.cursor() as cur:
        cur.execute("""INSERT INTO core.geography (geography_type, geoid, name, state_fips, county_fips)
          ...
          ON CONFLICT (geography_type, geoid) DO UPDATE SET name=EXCLUDED.name, state_fips=EXCLUDED.state_fips, county_fips=EXCLUDED.county_fips""", (layers,))
        cur.execute("""INSERT INTO core.geography_boundary (geography_id, boundary_vintage, geom, source_artifact_id)
          ...
          ON CONFLICT (geography_id, boundary_vintage) DO UPDATE SET geom=EXCLUDED.geom, source_artifact_id=EXCLUDED.source_artifact_id""", (vintage, layers))
        total = cur.rowcount; conn.commit()
    return total
