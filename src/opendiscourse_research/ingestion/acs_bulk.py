...
def _table_id(resource: dict[str, Any]) -> tuple[int, str] | None:
    ...
    if resource.get("dataset_id") != "census.acs_5" or resource.get("resource_type") != "Detailed Table":
        return None
    try:
...
def preview_acs5_bulk_plan(path: Path, update: Callable[[str], None] | None = None) -> dict[str, Any]:
    ...
    report = storage_preview(objects, stage_multiplier=float(storage.get("stage_multiplier", 1.0)), database_multiplier=float(storage.get("database_multiplier", 1.5)), reserve_bytes=int(storage.get("reserve_gib", 100)) * GiB)
