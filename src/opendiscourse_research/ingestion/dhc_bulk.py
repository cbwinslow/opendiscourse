...
def build_dhc_bulk_plan(basket_name: str, resources: list[dict[str, Any]]) -> dict[str, Any]:
    """Create a review-only plan for the complete 2020 DHC national archive."""
    matching = [item for item in resources if item.get("dataset_id") == "census.decennial" and item.get("resource_key") == "dhc:2020:national"]
...
def write_dhc_bulk_plan(basket_name: str, resources: list[dict[str, Any]]) -> Path:
    path = _root() / f"dhc-{basket_name}.yaml"; temp = path.with_suffix(".yaml.part")
    temp.write_text(yaml.safe_dump(build_dhc_bulk_plan(basket_name, resources), sort_keys=False)); temp.replace(path)
    return path
...
def preview_dhc_bulk_plan(path: Path, update: Callable[[str], None] | None = None) -> dict[str, Any]:
    ...
    storage = plan["storage"]; report = storage_preview(objects, stage_multiplier=float(storage["stage_multiplier"]), database_multiplier=float(storage["database_multiplier"]), reserve_bytes=int(storage["reserve_gib"]) * GiB)
