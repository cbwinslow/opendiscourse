...
def ingest_series(series_id: str) -> int:
    ...
    with client() as http, IngestionRun("fred.series", {"series_id": series_id}) as run:
        response = http.get("https://api.stlouisfed.org/fred/series/observations", params=params)
        ...
        for observation in payload["observations"]:
            ...
            with run.conn.cursor() as cur:
                cur.execute(
                    ...
                    (series_id, observation["date"], observation["realtime_start"], value, '{"source":"FRED"}', payload_id),
                )
                run.record_count += 1
            run.conn.commit()
...
def ingest_manifest(category: str | None = None, priority: int | None = None) -> dict[str, int]:
    ...
    selected = [entry for entry in series if (category is None or entry["category"] == category) and (priority is None or entry["priority"] <= priority)]
