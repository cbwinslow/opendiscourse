...
def _scope(plan: dict[str, Any]) -> set[str]:
    ...
    if unknown or not levels:
        raise ValueError(f"Choose one or more supported PEP geography levels (nation, state, county); got {sorted(unknown)}")
    return levels
...
def stage_pep(plan: dict[str, Any], update: Callable[[str], None] | None = None) -> int:
    ...
    artifacts = [("state", f"pep-{year}-state-totals"), ("county", f"pep-{year}-county-totals")]
    total = 0
    with connect() as conn:
        for level, key in artifacts:
            if level not in requested and not (level == "state" and "nation" in requested):
                continue
            ...
            with Path(artifact["local_path"]).open(encoding="latin-1", newline="") as source:
                rows = []
                for ordinal, row in enumerate(csv.DictReader(source), start=1):
                    sumlev = row.get("SUMLEV", "")
                    geography_level = "county" if sumlev == "050" else "state" if sumlev == "040" else "nation" if sumlev == "010" else "other"
                    if geography_level not in requested:
                        continue
                    rows.append((artifact["artifact_id"], Path(artifact["local_path"]).name, ordinal, geography_level, Jsonb(row)))
                    if len(rows) == 2_000:
                        with conn.cursor() as cur:
                            cur.executemany("INSERT INTO stage.pep_row (artifact_id, source_member, source_ordinal, geography_level, raw) VALUES (%s, %s, %s, %s, %s) ON CONFLICT DO NOTHING", rows)
                        total += len(rows); rows = []
                if rows:
                    with conn.cursor() as cur:
                        cur.executemany("INSERT INTO stage.pep_row (artifact_id, source_member, source_ordinal, geography_level, raw) VALUES (%s, %s, %s, %s, %s) ON CONFLICT DO NOTHING", rows)
...
def load_pep(plan: dict[str, Any], update: Callable[[str], None] | None = None) -> int:
    ...
    with connect() as conn, conn.cursor() as cur:
        cur.execute("""WITH source AS (
          ...
          ON CONFLICT (geography_type, geoid) DO UPDATE SET name = COALESCE(core.geography.name, EXCLUDED.name), state_fips = COALESCE(core.geography.state_fips, EXCLUDED.state_fips), county_fips = COALESCE(core.geography.county_fips, EXCLUDED.county_fips)""", (levels,))
        if update:
        ...
        cur.execute("""WITH source AS (
          ...
          ON CONFLICT (source_artifact_id, source_member, source_ordinal, estimate_year) DO UPDATE SET population = EXCLUDED.population""", (levels, vintage))
        total = cur.rowcount
        conn.commit()
    return total
