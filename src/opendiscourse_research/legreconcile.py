    ...
    for group_index, group in enumerate(groups, start=1):
        ...
        with zipfile.ZipFile(group["archive"]) as archive:
            ...
            for member in members:
                ...
                if (
                    item is None
                    or item["congress"] != congress
                    or item["bill_type"] != bill_type
                ):
                    malformed.append(
                        f"{group['archive']}:{member}: invalid bill identity"
                    )
                    continue
                processed += 1
                metrics["xml_bills"] += 1
                metrics["canonical_bill_match"] += int(item["bill_number"] in canonical)
                metrics["canonical_bill_missing"] += int(
                    item["bill_number"] not in canonical
                )
        ...
        if report:
            report(
                f"Reconciling BILLSTATUS ({group_index}/{len(groups)}): {congress} {bill_type}; {processed} XML bills inspected"
            )
        if limit is not None and processed >= limit:
            break
    result = {
