...
def pending_archive_files(
    listing: dict[str, Any], members: set[str]
) -> list[dict[str, Any]]:
...
def backfill_billstatus_missing(
    congress: int = 119, report: Callable[[str], None] | None = None
) -> dict[str, Any]:
    ...
    if (
        ...
        or contract.get("approval") != "approved_missing_file_backfill"
    ):
        raise ValueError("119th BILLSTATUS backfill contract is not approved")
    manifest_path = (
        _meta_path("plan", "govinfo") / f"billstatus-{congress}-missing.json"
    )
    manifest = json.loads(manifest_path.read_text())
    if manifest.get("congress") != congress or not manifest.get("storage", {}).get(
        "approved"
    ):
        ...
        with (
            ...
            for item in pending.values():
                ...
                if item.get("bytes") is not None and len(content) != int(item["bytes"]):
                    raise ValueError(
                        f"Size mismatch for {name}: expected {item['bytes']}, got {len(content)}"
                    )
                ...
                if name not in known:
                    listing["files"].append(
                        {"name": name, "link": url, "size": item.get("bytes")}
                    )
                    known.add(name)
                else:
                    repaired += 1
                ...
                if report:
                    report(
                        f"Backfilling BILLSTATUS ({position}/{len(groups)}): {bill_type}; {downloaded}/{len(manifest['files'])} files"
                    )
