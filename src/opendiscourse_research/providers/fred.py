...
def search(text: str, limit: int = 100) -> int:
    ...
    return cache_fred_search(
        _get("series/search", search_text=query, limit=min(limit, 1000)).get(
            "seriess", []
        ),
        query,
    )
...
def index_batch(
    ...
    if prior is None:
        raise ValueError(
            "FRED metadata indexing is already running; wait for it to finish or retry after 15 minutes"
        )
    ...
    while pages is None or completed < pages:
        if (
            ...
            and completed
            and monotonic() - started >= duration_seconds
        ):
            break
        release_offset = int(cursor.get("release_offset", 0))
        release_id = cursor.get("release_id")
        if release_id is None:
            releases = _get("releases", limit=1, offset=release_offset).get(
                "releases", []
            )
            if not releases:
                save_discovery(
        ...
        payload = _get(
            "release/series", release_id=release_id, limit=1000, offset=offset
        )
        records = payload.get("seriess", [])
        cached = cache_fred_records(
            records, {"method": "release", "release_id": release_id}
        )
