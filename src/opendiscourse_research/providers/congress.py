...
CONGRESS_RESOURCES = (
    ...
    ("housevotes", "House votes", "Congress.gov House roll-call metadata and member positions."),
)
...
def _summary_path() -> Path:
    ...
    return Path(settings.data_root).expanduser().resolve().parent / "meta" / "audit" / "leg" / "summary.json"
...
def sync() -> dict[str, Any]:
    ...
    audit = _audit()
    if audit is None:
        return {"state": "needs_audit", "next": "Run `research-db audit` before browsing legacy legislative coverage."}
    api_resources = 0
    for congress in range(1, 120):
        for resource_type, title, summary in CONGRESS_RESOURCES:
            upsert_resource(
                "congress.legislation", f"api:{congress}:{resource_type}", resource_type,
                f"{congress}th Congress — {title}", summary, congress,
                {"source": "Congress.gov API", "congress": congress, "coverage": "official_candidate", "ingest_state": "draft_only"},
            )
            api_resources += 1
    ...
    for root in audit["roots"]:
        ...
        for key, files in root.get("coverage", {}).items():
            ...
            entry = coverage.setdefault((collection, congress), {"files": 0, "roots": []})
            entry["files"] += files
    ...
    for congress in range(1, 120):
        for collection in GOVINFO_COLLECTIONS:
            ...
            if local:
                metadata.update({"legacy_files": local["files"], "legacy_roots": local["roots"], "provenance": "legacy_cache_unverified", "ingest_state": "verify_required"})
                summary = f"Official bulk candidate; {local['files']} audited legacy files are available for verification first."
            upsert_resource(
                "govinfo.bulk", f"govinfo:{collection}:{congress}", collection,
                f"{congress}th Congress — GovInfo {collection}",
                summary, congress, metadata,
            )
            bulk_resources += 1
            if collection == "BILLSTATUS":
                upsert_resource(
                    "congress.govinfo_billstatus", f"govinfo:BILLSTATUS:{congress}", "BILLSTATUS",
                    f"{congress}th Congress — GovInfo BILLSTATUS", summary, congress, metadata,
                )
                billstatus_resources += 1
    return {"state": "synced", "audit": str(_summary_path()), "congress_api_resources": api_resources,
            "govinfo_candidate_resources": bulk_resources, "billstatus_candidate_resources": billstatus_resources,
