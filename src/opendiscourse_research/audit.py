...
def audit_leg(
    hash_files: bool = False, report: Callable[[str], None] | None = None
) -> dict[str, Any]:
    ...
    for position, (root_id, path, dataset_id, provenance) in enumerate(
        all_files, start=1
    ):
        stat = path.stat()
    ...
    result = {
        ...
        "summary": {
            ...
            "formats": dict(
                sorted(Counter(entry["format"] for entry in entries).items())
            ),
            "congresses": dict(
                sorted(
                    Counter(
            ...
            "collections": dict(
                sorted(
                    Counter(
                        entry["collection"] for entry in entries if entry["collection"]
                    ).items()
                )
            ),
        },
