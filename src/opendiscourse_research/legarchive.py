...
def billstatus_groups(
    congress: int, *, allow_partial: bool = False
) -> list[dict[str, Any]]:
    ...
    if not report.is_file():
        raise FileNotFoundError(
            "Run `research-db validate billstatus --official --all` before loading"
        )
    ...
    if not comparisons:
        raise ValueError(
            f"No official BILLSTATUS validation is available for Congress {congress}"
        )
    incomplete = [
        kind
            ...
            groups.append(
                {
                    ...
                    "archive": archive,
                    "coverage": "complete"
                    if comparisons[bill_type].get("matches_official")
                    else "partial",
    ...
    if not groups:
        raise ValueError(
            f"No local BILLSTATUS archives are available for Congress {congress}"
        )
    return groups
