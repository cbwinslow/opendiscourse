...
def _acs_manifest(year: int) -> Path:
    path = (
        Path(settings.data_root).resolve().parent
        / "meta"
        / "acs"
        / str(year)
        / "tables.json"
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"No ACS manifest for {year}; run `research-db ingest census-discover --year {year}` first"
        )
    return path
...
def sync_acs(year: int) -> int:
    ...
    with session() as active_session:
        ...
        artifact = get_current_artifact(f"tables-{year}", dataset_id="census.acs_5")
...
def _fred_categories() -> list[dict[str, Any]]:
    ...
    while pending:
        ...
        for category in _fred_get("category/children", category_id=parent_id).get(
            "categories", []
        ):
...
def preview_fred_full() -> dict[str, Any]:
    ...
    for category in categories:
        payload = _fred_get(
            "category/series", category_id=category["id"], limit=1, offset=0
        )
...
def sync_fred_full() -> dict[str, Any]:
    ...
    for category in categories:
        ...
        while True:
            payload = _fred_get(
                "category/series", category_id=category["id"], limit=1000, offset=offset
            )
...
def facets(dataset_id: str) -> dict[str, Any]:
    ...
    if dataset_id == "census.acs_5":
        ...
        years = [
            {"release_year": year, "count": counts.get(year, 0)}
...
def add_acs_package(name: str, release_year: int, package: str = "housing_core") -> int:
    ...
    if release_year < 2022:
        raise ValueError(
            "Reviewed ACS bulk packages require a 2022 or later table-based release"
        )
...
def draft(name: str) -> Path:
    ...
    for item in items:
        groups.setdefault(
            (item["dataset_id"], item["release_year"], item["resource_type"]), []
        ).append(item["resource_key"])
    payload = {
        "version": 1,
        "state": "draft",
        "basket": name,
    ...
    path = (
        Path(settings.data_root).resolve().parent / "meta" / "drafts" / f"{name}.yaml"
    )
...
def launch(
    ...
    try:
        ...
        raise RuntimeError(
            "Install the optional browser: `pip install -e '.[browser]'`"
        ) from exc
    ...
    class CatalogBrowser(App):
        ...
        def compose(self) -> ComposeResult:
            ...
            yield Static(
                "↑/↓ move  Enter or O open  Backspace back  Space select  C selection  F fetch FRED search  A select filtered  G write draft  R refresh  Ctrl+Q quit",
                id="help",
            )
            yield Footer()
        ...
        def _set_initial_row(self) -> None:
            ...
            if not self.rows:
                ...
                return
            self.current_id = str(
                self.rows[0].get("resource_id")
                or self.rows[0].get("provider_id")
                or self.rows[0].get("dataset_id")
                or self.rows[0].get("release_year")
                or self.rows[0].get("resource_type")
            )
        ...
        def _help(self) -> str:
            ...
            if self.level == "resource":
                ...
                if (
                    self.dataset_id == "census.acs_5"
                    and self.product == "Detailed Table"
                    and self.year
                ):
        ...
        def load_level(self) -> None:
            ...
            if self.level == "provider":
                ...
                for row in self.rows:
                    table.add_row(
                        row["provider_id"], row["name"], key=row["provider_id"]
                    )
                ...
                for row in catalog_years:
                    table.add_row(
                        str(row["release_year"]),
                        str(row["count"]),
                        key=str(row["release_year"]),
                    )
            ...
            self.query_one(Static).update(
                f"{self.crumb()} · selection {len(basket(basket_name))} · Enter to continue, Backspace to return."
            )
        ...
        def load_rows(self, query: str = "") -> None:
            ...
            for row in self.rows:
                ...
                table.add_row(
                    "●" if str(row["resource_id"]) in selected else "",
                    metadata.get("table_id", row["resource_key"]),
                    row["resource_type"],
                    row["title"],
                    key=str(row["resource_id"]),
                )
            self._set_initial_row()
            self.query_one(Static).update(
                f"{len(self.rows)} resources. Space selects; Enter fetches field metadata for the highlighted resource."
            )

        def on_input_changed(self, event: Input.Changed) -> None:
            if self.level == "resource":
                self.load_rows(event.value)
        ...
        def action_toggle(self) -> None:
            ...
            if self.level in {"year", "product"}:
                ...
                if self.confirm != marker:
                    self.confirm = marker
                    self.query_one(Static).update(
                        f"Press Space again to add all {len(ids)} resources in this {self.level} to the selection."
                    )
                    return
                ...
                self.query_one(Static).update(
                    f"Added {len(ids)} resources from this {self.level} to the selection."
                )
                return
            if self.level != "resource":
                return
            ...
            self.query_one(Static).update(
                "Added to selection." if selected else "Removed from selection."
            )
        ...
        def action_describe(self) -> None:
            ...
            if self.level != "resource":
                if self.level == "provider":
                    self.provider_id, self.level = self.current_id, "dataset"
                elif self.level == "dataset":
                    ...
                    self.level = (
                        "year" if facets(self.dataset_id)["years"] else "product"
                    )
                elif self.level == "year":
                    self.year, self.level = int(self.current_id), "product"
                    if (
                        self.dataset_id == "census.acs_5"
                        and not _acs_manifest(self.year).is_file()
                    ):
                        ensure_acs(self.year)
                elif self.level == "product":
                    self.product, self.level = self.current_id, "resource"
                self.load_level()
                return
            resource = get_resource(self.current_id)
            if resource["dataset_id"] == "census.api_catalog":
                ...
                lines = [
                    resource["title"],
                    "",
                    resource.get("summary") or "No provider description published.",
                ]
                for label, key in (
                    ("API endpoint", "endpoint"),
                    ("Variables", "variables_url"),
                    ("Groups", "groups_url"),
                    ("Geography", "geography_url"),
                ):
                    if metadata.get(key):
                        lines.extend(["", f"{label}: {metadata[key]}"])
                lines.extend(
                    ["", "Catalog metadata only — selection does not download data."]
                )
                self.query_one(Static).update("\n".join(lines))
                return
            if resource["dataset_id"] != "census.acs_5":
                self.query_one(Static).update(
                    "Field discovery is not yet implemented for this provider."
                )
                return
            ...
            try:
                ...
                lines = [
                    f"{report['table']['id']} — {report['table']['title']}",
                    report["table"].get("universe") or "Universe not published",
                    "",
                    "Fields:",
                ]
                lines.extend(
                    f"{field['id']}  {field['label']}" for field in report["fields"]
                )
                self.query_one(Static).update("\n".join(lines))
        ...
        def action_all(self) -> None:
            ...
            if self.confirm != "all":
                self.confirm = "all"
                self.query_one(Static).update(
                    f"Press A again to select exactly {len(self.rows)} filtered resources."
                )
                return
            add_all(basket_name, [str(row["resource_id"]) for row in self.rows])
            self.confirm = None
            self.query_one(Static).update(
                f"Selected {len(self.rows)} currently filtered resources."
            )
            self.load_rows(self.query_one(Input).value)
        ...
        def _add_acs_housing_package(self, package: str, label: str) -> None:
            ...
            if (
                self.level != "resource"
                or self.dataset_id != "census.acs_5"
                or self.product != "Detailed Table"
                or not self.year
            ):
                self.query_one(Static).update(
                    f"Open an ACS release's Detailed Table product to add the reviewed {label} package."
                )
                return
            ...
            if self.confirm != marker:
                self.confirm = marker
                self.query_one(Static).update(
                    f"Press the same key again to add the seven reviewed ACS {label} tables for this release."
                )
                return
            try:
            ...
            self.confirm = None
            self._debug(
                "acs_package_selected",
                package=package,
                release_year=self.year,
                count=count,
            )
            self.query_one(Static).update(
                f"Added {count} ACS {label} tables. Press P to write its bulk plan."
            )
            self.load_rows(self.query_one(Input).value)
        ...
        def action_fetch(self) -> None:
            ...
            if self.level != "resource" or self.dataset_id != "fred.series":
                self.query_one(Static).update(
                    "Provider search is available after opening FRED resources."
                )
                return
            ...
            if len(query.strip()) < 2:
                ...
                self.query_one(Static).update(
                    "Type at least two characters, then press Enter to search official FRED metadata."
                )
                return
        ...
        def _fetch_fred(self, query: str) -> None:
            ...
            self.query_one(Static).update(
                f"Cached {count} FRED search results. Space selects; G writes a disabled draft."
            )
        ...
        def action_draft(self) -> None:
            if self.confirm != "draft":
                self.confirm = "draft"
                self.query_one(Static).update(
                    "Press G again to write a disabled draft contract."
                )
                return
        ...
        def action_bulk_plan(self) -> None:
            ...
            if self.confirm != "bulk_plan":
                ...
                self.query_one(Static).update(
                    "Press P again to write an explicit, disabled Census bulk plan from one selected complete package."
                )
                return
            ...
            from .ingestion.tiger_bulk import write_tiger_bulk_plan
            ...
            try:
                ...
                elif datasets == {"census.tiger"}:
                    path = write_tiger_bulk_plan(basket_name, selected)
                    command = "tiger-bulk-preview"
                else:
                    path = write_acs5_bulk_plan(basket_name, selected)
                    command = "acs-bulk-preview"
            except Exception as exc:
                self.query_one(Static).update(
                    f"Could not write Census bulk plan: {exc}"
                )
                return
            self.confirm = None
            self._debug(
                "census_bulk_plan_written",
                path=str(path),
                dataset=next(iter(datasets), None),
            )
            self.query_one(Static).update(
                f"Wrote disabled Census bulk plan: {path}\nRun `research-db ingest {command} --plan {path}` to measure every artifact."
            )
        ...
        def action_back(self) -> None:
            ...
            if self.level == "cart":
                ...
                self.level == "product"
                and self.dataset_id
                and not facets(self.dataset_id)["years"]
            ):
                self.level = "dataset"
