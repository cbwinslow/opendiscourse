...
def _meta_path(*parts: str) -> Path:
    ...
    return (
        Path(settings.data_root).expanduser().resolve().parent / "meta" / Path(*parts)
    )
...
def _validated_groups(congress: int) -> list[dict[str, Any]]:
    ...
    if not report.is_file():
        raise FileNotFoundError(
            "Run `research-db validate billstatus --official --congress N` first"
        )
    ...
    if len(groups) != 8 or any("error" in group for group in groups):
        raise ValueError(
            f"No complete official BILLSTATUS comparison is available for Congress {congress}"
        )
    return groups


def plan_billstatus_backfill(
    ...
    for position, group in enumerate(groups, start=1):
        ...
        local = {
            item["name"]
            for item in local_listing["files"]
            if item.get("name", "").endswith(".xml")
        }
        ...
        if report:
            report(
                f"Planning missing BILLSTATUS files ({position}/8): {congress} {bill_type}"
            )
