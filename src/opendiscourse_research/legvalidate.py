...
def _validate_xml(
    content: bytes, congress: int, bill_type: str
) -> dict[str, Any] | None:
    ...
    if (
        found_congress != str(congress)
        or (found_type or "").lower() != bill_type
        or not number
    ):
        return None
...
def _official_compare(
    groups: list[dict[str, Any]], congress: int, report: Callable[[str], None] | None
) -> list[dict[str, Any]]:
    """Compare one Congress's cached listings to paced official GovInfo directory JSON."""
    compared: list[dict[str, Any]] = []
    last_request = 0.0
    for position, group in enumerate(
        (item for item in groups if item["congress"] == congress), start=1
    ):
        ...
        try:
            ...
            local = {
                item["name"]
                for item in json.loads(Path(group["listing"]).read_text())["files"]
                if item.get("name", "").endswith(".xml")
            }
            compared.append(
                {
        ...
        if report:
            report(
                f"Comparing official BILLSTATUS listings ({position}/8): {congress} {bill_type}"
            )
    return compared
...
def validate_billstatus(
    ...
    for index, listing in enumerate(listings, start=1):
                        ...
                        errors.append(
                            {
                                ...
                                "error": "invalid or mismatched bill identity",
                            }
                        )
                    else:
        ...
        if report:
            report(
                f"Validating BILLSTATUS groups ({index}/{len(listings)}): {congress} {bill_type}"
            )
    mismatch = [
        group for group in groups if group.get("listing_matches_archive") is False
    ]
    result = {
        "schema": 1,
        ...
        "summary": {
            ...
            "errors": len(errors),
            "by_congress": dict(
                sorted(Counter(group["congress"] for group in groups).items())
            ),
        },
        "errors": errors,
    }
    if official_congresses:
        ...
        archive_by_key = {
            (group["congress"], group["bill_type"]): group for group in groups
        }
        for comparison in result["official_comparison"]:
            archive = archive_by_key.get(
                (comparison["congress"], comparison["bill_type"]), {}
            )
