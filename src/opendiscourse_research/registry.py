...
from .providers.census import sync_acs_bulk_packages, sync_cbp_bulk_packages, sync_dhc_bulk_packages, sync_pep_bulk_packages, sync_catalog as sync_census_catalog
...
def sync(refresh: bool = False, sources: set[str] | None = None, full: bool = False, preview: bool = False, index_pages: int | None = None, index_seconds: int | None = None, report: Callable[[str], None] | None = None) -> dict[str, Any]:
    ...
    if "census" in requested:
        ...
        results["census"] = {**catalog_result, "acs_bulk_packages": sync_acs_bulk_packages(), "cbp_bulk_packages": sync_cbp_bulk_packages(), "pep_bulk_packages": sync_pep_bulk_packages(), "dhc_bulk_packages": sync_dhc_bulk_packages()}
    if "acs" in requested:
        with connect() as conn, conn.cursor() as cur:
